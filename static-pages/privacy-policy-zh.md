...
- [Cloudflare](https://www.cloudflare.com/) 为域名 `robomico.cn` 提供 DNS 解析服务。Cloudflare 会收集并储存您在浏览时产生的流量数据信息（IP 属地、流量大小、请求次数等），详见 [Cloudflare 隐私方针](https://www.cloudflare.com/privacypolicy/)。
...
- 文章中嵌入的图片和所有可下载的文件由 OneDrive 托管。在您浏览图片以及下载文件时，微软会读取并记录您浏览器所保存的微软帐号会话信息。
